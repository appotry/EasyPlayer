...
DWORD WINAPI ThreadRecord(LPVOID lpParam)
{
	...
	FFE_SetAudioEncodeParam(pDSoundObj->ffeAudioHandle, ENCODER_AAC, DS_CHANNEL, DS_SAMPLERATE, 16000);
	...
	while (1)
	{
		...
		case WAIT_OBJECT_0 + 0:
			{
				if (! pThis->RecordCapturedData())	break;
			}
			break;
...
CDirectSound::CDirectSound()
{
	...
	m_DSoundObj.pcmdata_size = DS_BUFFER_SIZE;
	m_DSoundObj.pcmdata = new unsigned char[DS_BUFFER_SIZE];
	m_DSoundObj.pcmdata_pos = 0;

	m_DSoundObj.aacdata_size = DS_BUFFER_SIZE;
	m_DSoundObj.aacdata = new unsigned char[DS_BUFFER_SIZE];
...
{
	INT nCaptureIndex = (INT)SendMessage(hComboWnd, CB_GETCURSEL, 0, 0);
	GUID *pCaptureGUID = (GUID *)SendMessage( hComboWnd, CB_GETITEMDATA, nCaptureIndex, 0);
...
BOOL CDirectSound::CreateCaptureBuffer(WAVEFORMATEX *pwfxInput)
{
	...
	if (FAILED(m_DSoundObj.m_pDSCapture->CreateCaptureBuffer(&dscbd, &m_DSoundObj.m_pDSBCapture, NULL)))
	{
		return FALSE;
	}
...
BOOL CDirectSound::Start()
{
	if (m_DSoundObj.flag == 0x00)
	{
		...
		if (FAILED(m_DSoundObj.m_pDSBCapture->GetFormat(&wfxInput, sizeof(wfxInput), NULL)))
		{
			return FALSE;
		}
...
*/
		...
		if (NULL != m_DSoundObj.pCallback)
		{
			...
			int enc_size = 0;
			int ret = FFE_EncodeAudio(m_DSoundObj.ffeAudioHandle, (unsigned short*)m_DSoundObj.pcmdata, m_DSoundObj.pcmdata_pos, (unsigned char*)m_DSoundObj.aacdata, &enc_size);
			if (ret == 0x00 && enc_size>0)
			{
				...
				frameinfo.timestamp_usec = uiAudioPts * 1000;
