...
bool CCameraDS::OpenCamera(int nCamID, int nWidth, int nHeight, MediaSourceCallBack _callback, void *userPtr)
{
	...
	while (cameraSourceThread.flag!=0x02 && cameraSourceThread.flag!=0x00)	{Sleep(200);}
...
{
	if (cameraSourceThread.flag == 0x00)		return -1;
	return 0;
}
...
DWORD WINAPI __CameraProcDataThread(LPVOID lpParam)
{
	...
	while (1)
	{
		...
		pThread->m_pSampleGrabber->GetCurrentBuffer(&size, NULL);


		pThread->m_pSampleGrabber->GetCurrentBuffer(&size, (long*)raw_data);
		...
		if (ret == 0x00 && enc_size>0)
		{
			...
			//frameinfo.timestamp_usec = uiPts * 1000;
			//uiPts += 80;
