...
extern "C"
{
	...
	int	 NVSOURCE_API	NVS_OpenStream(NVS_HANDLE handle, int _channelid, char *_url, RTP_CONNECT_TYPE _connType, unsigned int _mediaType, char *_username, char *_password, void *userPtr, int _reconn/*1000表示长连接,即如果网络断开自动重连, 其它值为连接次数*/, int outRtpPacket/*默认为0,即回调输出完整的帧, 如果为1,则输出RTP包*/, double starttime=0.0f, double endtime=0.0f, float fScale=0.0f);
	int	 NVSOURCE_API	NVS_CloseStream(NVS_HANDLE handle);

	int	 NVSOURCE_API	NVS_FastPlay(NVS_HANDLE handle, float fScale);
	int	 NVSOURCE_API	NVS_Seek(NVS_HANDLE handle, double starttime, double endtime, float fScale);

	int	 NVSOURCE_API	NVS_PauseStream(NVS_HANDLE handle);
	int	 NVSOURCE_API	NVS_ResumeStream(NVS_HANDLE handle);
