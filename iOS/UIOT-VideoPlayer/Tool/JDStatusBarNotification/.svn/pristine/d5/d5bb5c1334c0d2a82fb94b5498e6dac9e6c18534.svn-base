                    ...
                    style:(JDStatusBarStyle*)style;
{
    ...
    if (style != self.activeStyle) {
        ...
        if (self.activeStyle.animationType == JDStatusBarAnimationTypeFade) {
            ...
            self.topBar.transform = CGAffineTransformMakeTranslation(0, 40);
        }
    }
...
- (void)dismissAnimated:(BOOL)animated;
{
    ...
    // animate out
    CGFloat yPos = 0;
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0 && self.topBar.frame.size.height > 20.0) {
        yPos = -self.topBar.frame.size.height/2.0;
    }
...
- (void)updateTopBarFrameWithStatusBarFrame:(CGRect)rect;
{
    ...
    _topBar.frame = CGRectMake(0, ([UIScreen mainScreen].bounds.size.height-yPos)/5*4, width, height);
}
